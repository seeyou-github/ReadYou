 ...
 */
@Singleton
class StreamCerebrasTranslate @Inject constructor(
    private val translateCache: TranslateCache
) : StreamTranslateService {
    companion object {
        private const val TAG = "StreamCerebras"
        private const val API_URL = "https://api.cerebras.ai/v1/chat/completions"
...
6. 只输出翻译结果，不要有任何解释或思考过程"""
        private const val SERVICE_ID = "cerebras"
        private const val SERVICE_NAME = "Cerebras"
    ...
    private fun getApiKey(config: TranslateModelConfig?): String {
        ...
        if (apiKey.isNullOrBlank()) {
            throw IllegalArgumentException(
                "API Key 未配置，请在设置中配置 Cerebras API Key"
            )
        }
        return apiKey
    }
    ...
    private fun getModel(config: TranslateModelConfig?): String {
        ...
        if (model.isNullOrBlank()) {
            throw IllegalArgumentException(
                "翻译模型未配置，请在设置中选择或配置 Cerebras 模型"
            )
        }
        return model
    }
...
//        Timber.d("[$TAG] 合并文本预览: ${mergedText}")
        ...
        val requestBody = JsonObject().apply {
            ...
            addProperty("reasoning_format", "hidden")
